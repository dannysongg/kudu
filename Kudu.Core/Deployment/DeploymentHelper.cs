...
namespace Kudu.Core.Deployment
{
    public static class DeploymentHelper
    {
        ...
        public static async Task<HttpContent> GetArtifactContentFromURL(ArtifactDeploymentInfo artifactDeploymentInfo, ITracer tracer)
        {
            ...
            {
                Uri uri = new Uri(artifactDeploymentInfo.ArtifactURL);
                using (tracer.Step($"Trying to make a GET request to {StringUtils.ObfuscatePath(artifactDeploymentInfo.ArtifactURL)}"))
                {
                    try
                    {
